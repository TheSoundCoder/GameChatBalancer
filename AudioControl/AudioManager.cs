...
namespace AudioManager
{
    ...
    public static class AudioManager
    {
        ...
        public static int[] GetApplicationPIDByName(string AppName)
        {
            ...
            try
            {
                ...
                // activate the session manager. we need the enumerator
                Guid IID_IAudioSessionManager2 = typeof(IAudioSessionManager2).GUID;
                //RetVal = IID_IAudioSessionManager2.ToString();
                object o;
                speakers.Activate(ref IID_IAudioSessionManager2, 0, IntPtr.Zero, out o);
                mgr = (IAudioSessionManager2)o;
                ...
                for (int i = 0; i < count; ++i)
                {
                    ...
                    if (cAppName.ToUpper() == AppName.ToUpper())
                    {
                        Array.Resize(ref RetVal, RetVal.Length + 1);
                        RetVal[RetVal.Length - 1] = cpid;

                        //RetVal.Append(cpid);
                        //RetVal += cAppName + "(" + cpid.ToString() + ")" + "\r\n";
                    }
        ...
        public static string GetAudioApplications(bool IncludingPID)
        {
            string RetVal = "";
            int cpid;
            string cAppName;
        ...
        /*
        public static void SetApplicationVolumeByName(string AppName, float level)
        {
            int cpid;
            string cAppName;
            //IMMDeviceEnumerator deviceEnumerator = null;
            ...
            try
            {
                ...
                for (int i = 0; i < count; ++i)
                {
                    ...
                    try
                    {
                        ...
                        ctl.GetProcessId(out cpid);
                        cAppName = Process.GetProcessById(cpid).ProcessName;
                        if (AppName.ToUpper().Contains(cAppName.ToUpper()))
                        {
                            Guid guid = Guid.Empty;
                            volumeControl = ctl as ISimpleAudioVolume;
